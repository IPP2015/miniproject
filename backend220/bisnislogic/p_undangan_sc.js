...
const p_undangan_sc = {
        ...
        readp_undangan_scAllHandler: (req,res,next) => {
        dtl.readp_undangan_scAllHandlerData(function(items){
        responseHelper.sendResponse(res,200,items)
        })
        },
        insertp_undangan_scAllHandler:(req,res,next) => {
        var docs = req.body
        dtl.insertp_undangan_scAllHandlerData(function(items){
        responseHelper.sendResponse(res,200,items)
        },docs)
        },
        updatep_undangan_scAllHandler:(req,res,next) => {
            var docs = req.body
            dtl.updatep_undangan_scAllHandlerData(function(items){
            responseHelper.sendResponse(res,200,items)
            },docs)
            },
            deletedp_undangan_scAllHandler:(req,res,next) => {
                var docs = req.body
                dtl.deletedp_undangan_scAllHandlerData(function(items){
                responseHelper.sendResponse(res,200,items)
                },docs)
                },
